...
public final class EmpleadoPostgreSQLDAO extends SQLconnection implements EmpleadoDAO {
    ...
    public final List<EmpleadoEntity> consultarEmpleado(final EmpleadoEntity entidad) {
        final var listaEmpleado = new ArrayList<EmpleadoEntity>();
        final var sentenciaSql = new StringBuilder();
        sentenciaSql.append("SELECT Identificador, NumeroDocumento, Clave, Nombre, Telefono, Correo, Finca, Estado ");
        sentenciaSql.append("FROM Empleado ");
        sentenciaSql.append("WHERE NumeroDocumento = ?");
        sentenciaSql.append("AND Estado = 'Activo'");
        
        try (final PreparedStatement sentenciaPreparada = getConnection().prepareStatement(sentenciaSql.toString())){
            sentenciaPreparada.setInt(1, entidad.getDocumento());
            try (final ResultSet resultado = sentenciaPreparada.executeQuery()){
                List<EmpleadoEntity> Empleados = new ArrayList<>();
                while (resultado.next()){
                    EmpleadoEntity EmpleadoTMP = EmpleadoEntity.Build(resultado.getInt("Identificador"), resultado.getInt("NumeroDocumento"), resultado.getString("Estado"));
                    listaEmpleado.add(EmpleadoTMP);
                }
            }
        } catch (final SQLException exception){
            var mensajeUsuario = "No ha sido posible llevar a cabo la consulta del empleado. Por favor intente de nuevo y en caso de persistir el problema comuníquese con el administrador de la app tiendaChepito";
            var mensajeTecnico = MessageCatalogStrategy.getContenidoMensaje(CodigoMensaje.M000023);
            throw new DataFinKException(mensajeTecnico, mensajeUsuario, exception);
        } catch (final DataFinKException exception){
            throw exception;
